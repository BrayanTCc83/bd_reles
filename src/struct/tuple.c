...
tuple_t *clone_tuple(const tuple_t tuple) {
	return new_tuple(
		tuple.type,
		clone_object(tuple.value1),
		clone_object(tuple.value2)
	);
}
...
compare_result_t compare_tuple(tuple_t tuple1, tuple_t tuple2) {
	...
	if(result == DIFFERENT)
		return result;
	result = compare_objects(tuple1.value1, tuple2.value1);
	if(result == EQUALS)
		result = compare_objects(tuple1.value2, tuple2.value2);
	return result;
}
...
char *tuple_to_string(tuple_t tuple) {
	...
	sprintf(string, "T<%s, %s>", to_string(tuple.value1), to_string(tuple.value2));
	return string;
}