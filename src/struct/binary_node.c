...
binary_node_t *clone_binary_node(const binary_node_t node) {
	binary_node_t *BinaryNode = new_binary_node(clone_object(node.type, node.value), node.type);
...
compare_result_t compare_binary_node(binary_node_t node1, binary_node_t node2) {
	...
	return compare_objects(node1.type, node1.value, node2.value);
}
...
char *binary_node_to_string(binary_node_t node) {
	...
	sprintf(string, "%s -> N<%p, %p>", to_string(node.type, node.value), node.left, node.right);
	return string;
}
